...
const getLedger = async (req, res) => {
  try {
    ...
    const summary = {
      totalSales: ledgerEntries.reduce(
        (sum, order) => sum + (order.finalAmount || 0),
        0
      ),
      totalOrders: ledgerEntries.length,
      totalRefunds: ledgerEntries.reduce(
        (sum, order) => sum + (order.refundAmount || 0),
        0
      ),
      netRevenue: ledgerEntries.reduce(
        (sum, order) =>
          sum + (order.finalAmount || 0) - (order.refundAmount || 0),
        0
      ),
    };
...
const loadsalesreport = async (req, res) => {
  try {
    ...
    res
      ...
      .send("Error generating sales report");
  }
};
...
const downloadSalesReport = async (req, res) => {
  try {
    ...
    const totalCouponDiscount = orders.reduce((sum, order) => {
      return (
        sum +
        (order.coupon ? (order.finalAmount * order.coupon.discount) / 100 : 0)
      );
    }, 0);
    ...
    if (format === "excel") {
      ...
      sheet.addRow([
        ...
        `Rs.${totalCouponDiscount.toFixed(2)}`,
      ]);
      sheet.addRow([
        "Average Order Value:",
